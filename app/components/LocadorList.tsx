...
export default function LocadorList() {
  const { data, error, isLoading } = useSWR('/api/locadores', fetcher);
  ...
  const locadores = Array.isArray(data) ? data : [];

  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {locadores.length === 0 && !isLoading ? (
        <div className="col-span-full text-center py-10 text-gray-500">
          Nenhum locador cadastrado ainda.
        </div>
      ) : (
        locadores.map((locador: any) => (
          <Link key={locador.id} href={`/locadores/${locador.id}/editar`}>
