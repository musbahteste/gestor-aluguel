...
export default function LocatarioForm({ locatarioId }: LocatarioFormProps) {
  ...
  useEffect(() => {
    // Se tem locatarioId, está em modo de edição
    if (locatarioId) {
      setIsEditing(true);
      const fetchLocatario = async () => {
        try {
          const res = await fetch(`/api/locatarios/${locatarioId}`);
          ...
          alert("Não foi possível carregar os dados do locatário.");
        }
      };
      fetchLocatario();
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const url = isEditing ? `/api/locatarios/${locatarioId}` : '/api/locatarios';
      ...
      alert("Não foi possível salvar o locatário.");
    } finally {
      setLoading(false);
    }
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
        ...
        <div className="flex justify-end gap-3 pt-4">
          ...
          <button type="submit" disabled={loading} className="flex items-center justify-center gap-2 bg-blue-600 text-white px-6 py-2.5 rounded-lg shadow-sm hover:bg-blue-700 transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed">
            <Save size={18} />
            {loading ? 'Salvando...' : isEditing ? 'Atualizar Locatário' : 'Salvar Locatário'}
          </button>
        </div>
      </form>
    </div>
